...
export LD_LIBRARY_PATH="$CJC_BIN_DIR/../runtime/lib/linux_x86_64_cjnative:$CPP_RUNTIME_LIB:$LD_LIBRARY_PATH"
export cjHeapSize=${cjHeapSize:-24GB}
TMP=$(mktemp -d "${TMPDIR:-/tmp}/rt_forwarddata.XXXXXX")
trap 'rm -rf "$TMP"' EXIT
SOURCE="$ROOT/src/rt.gc.forwarddata/ForwardDataManager.cj"
...
PROBE="$OUT/rt.gc.forwarddata.probe"; cp -a "$ROOT/src/rt.gc.forwarddata" "$PROBE"
...
"$SELFHOST_CJC" --package "$PROBE" --import-path "$OUT" --int-overflow wrapping -Woff unused \
    "$OUT/librt.heap.allocator.a" "$OUT/librt.gc.a" "$OUT/librt.sync.a" "$OUT/librt.base.a" "$OUT/native/"*.o \
...
HEAP="$CLOSURE/rt.gc.forwarddata.noheap"; cp -a "$ROOT/src/rt.gc.forwarddata" "$HEAP"
cp "$ROOT/test/parity/gc/forwarddata_noheap.cj" "$HEAP/ForwardDataNoHeap.cj"
temps="$CLOSURE/rt.gc.forwarddata.temps"; mkdir -p "$temps"
"$SELFHOST_CJC" --package "$HEAP" --output-type=staticlib --save-temps "$temps" \
    --int-overflow wrapping -Woff unused --import-path "$CLOSURE" --output-dir "$CLOSURE" -o librt.gc.forwarddata.a
PACKAGE_TEMPS+=("$temps")
...
"$CANGJIE_HOME/third_party/llvm/bin/llvm-link" --only-needed "${PRE[4]}" "${PRE[3]}" "${PRE[2]}" "${PRE[1]}" "${PRE[0]}" -o "$CLOSURE/pre.bc"
...
"$SELFHOST_CJC" --package "$ROOT/src/rt.gc.forwarddata" --target x86_64-w64-mingw32 \
    --output-type=staticlib --int-overflow wrapping -Woff unused --import-path "$WIN" --output-dir "$WIN" -o librt.gc.forwarddata.a
"$SELFHOST_CJC" --package "$ROOT/src/rt.gc.forwarddata" --target x86_64-w64-mingw32 -g \
    --output-type=staticlib --int-overflow wrapping -Woff unused --import-path "$WIN" --output-dir "$WIN" -o librt.gc.forwarddata.debug.a
