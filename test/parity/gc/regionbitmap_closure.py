...
try:
    ...
    operations = sorted(n for n in pre if 'RegionBitmapNoHeapRoot' in n)
    if len(operations) != 1: raise closure.ClosureError(f'operation roots={operations}')
    static = sorted(n for n in pre if n.startswith('_CGV5rt.gc'))
    ...
    allowed = (r'^external node$', r'^llvm\.', r'^CJ_MCC_', r'^CJ_MRT_', r'^__cangjie_',
        r'^__stack_chk_fail$', r'^_CNat', r'^_CNap', r'^_CGPat', r'^cj_atomic_u(16|64)_', r'^abort$',
    ...
    print(f'REGIONBITMAP_ROOTS operations=1 static_initializers={len(static)} status=PASS')
    print(f'REGIONBITMAP_NOHEAP_CLOSURE reachable_defs={len(reached)} scanned_pre={len(reached)} scanned_final={len(reached)} scanned_object={len(reached)} native_defs=5 missing=0 status=PASS')
    print(f'REGIONBITMAP_NOHEAP forbidden_alloc=0 forbidden_barrier=0 external_edges={len(external)} status=PASS')
except (closure.ClosureError, OSError) as e:
    print(f'REGIONBITMAP_CLOSURE FAIL error={e}', file=sys.stderr); sys.exit(1)
