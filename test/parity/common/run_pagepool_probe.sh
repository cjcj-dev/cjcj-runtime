...
# Compile all dedicated roots in their owning package. The closure includes the
# PagePool-related global initializers, notably mutex construction at the live
# singleton address, rather than excluding static initialization from the gate.
NOHEAP_SRC="$TMP/rt.common.noheap"
NOHEAP_TEMPS="$TMP/noheap_temps"
cp -a "$ROOT/src/rt.common" "$NOHEAP_SRC"
cp "$ROOT/test/parity/common/pagepool_noheap_probe.cj" "$NOHEAP_SRC/PagePoolNoHeapProbe.cj"
...
(cd "$TMP" && "$SELFHOST_CJC" --package "$NOHEAP_SRC" --output-type=staticlib \
    ...
    -Woff unused --output-dir "$TMP" -o pagepool_noheap.a)

PRE_BC=()
for bc in "$NOHEAP_TEMPS"/*.bc; do
    if [[ "$bc" != *.opt.bc ]]; then
        PRE_BC+=("$bc")
    fi
done
"$CANGJIE_HOME/third_party/llvm/bin/llvm-link" "${PRE_BC[@]}" -o "$TMP/noheap.pre.bc"
...
for object in "$NOHEAP_TEMPS"/*.o; do
    ...
    BEGIN { while ((getline symbol < symbols) > 0) keep[symbol] = 1 }
    /^[[:xdigit:]]+ <.*>:/ {
        name=$0; sub(/^.*</, "", name); sub(/>.*$/, "", name); emit=(name in keep)
    }
    emit { print }
...
if [[ $NOHEAP_FINAL_BC -eq 0 || $NOHEAP_OBJECTS -eq 0 || $FORBIDDEN_REFS -ne 0 ||
      $MCC_NEW_REFS -ne 0 || $NOHEAP_ILLEGAL_AS1_CASTS -ne 0 ]] ||
   ! grep -q 'PagePoolInitNoHeapRoot' "$TMP/noheap.closure.ll" ||
   ! grep -q 'PAGE_POOL_MUTEX_CONSTRUCTED' "$TMP/noheap.closure.ll"; then
    echo "PAGEPOOL_NOHEAP FAIL objects=$NOHEAP_OBJECTS final_bc=$NOHEAP_FINAL_BC forbidden=$FORBIDDEN_REFS mcc=$MCC_NEW_REFS" >&2
