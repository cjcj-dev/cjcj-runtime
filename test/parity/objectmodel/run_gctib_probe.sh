...
g++ "${CPP_FLAGS[@]}" -fPIC -c "$ROOT/test/parity/objectmodel/object_traversal_bridge.cpp" -o "$TMP/ObjectTraversalBridge.o"
...
"$SELFHOST_CJC" --package "$TMP/rt.objectmodel.probe" --int-overflow wrapping -Woff unused "$TMP/Atomic.o" "$TMP/GCTibShift.o" "$TMP/ObjectTraversalBridge.o" --link-option=-lstdc++ --link-option=-lgcc_s -o "$TMP/probe"
"$TMP/probe" > "$TMP/probe.txt"
cmp "$TMP/ref.txt" "$TMP/probe.txt"
cat "$TMP/probe.txt"
echo "GCTIB_TRANSCRIPT lines=$(wc -l < "$TMP/probe.txt") bytes=$(wc -c < "$TMP/probe.txt") sha256=$(sha256sum "$TMP/probe.txt" | awk '{print $1}') cmp=PASS"
...
echo 'GCTIB_PLATFORM target=Linux-OHOS compile=PASS execute=PASS status=PASS'
echo 'GCTIB_PLATFORM target=Apple source=PASS native_execute=DEBT-APPLE-SDK status=EXPLICIT-DEBT'
echo 'GCTIB_PLATFORM target=Win64 cj_release=PASS cj_debug=PASS status=PASS'
echo 'GCTIB_PLATFORM target=OHOS-arm32 source=PASS native_execute=DEBT-OHOS-ARM32-SDK status=EXPLICIT-DEBT'

cp -a "$ROOT/src/rt.objectmodel" "$TMP/rt.objectmodel.noheap"
cp "$ROOT/test/parity/objectmodel/gctib_noheap.cj" "$TMP/rt.objectmodel.noheap/NoHeap.cj"
mkdir -p "$TMP/temps"
