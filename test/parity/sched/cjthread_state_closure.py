...
SPEC = importlib.util.spec_from_file_location("cjthread_state_closure_engine", ENGINE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"cannot load closure engine: {ENGINE_PATH}")
ENGINE = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(ENGINE)

STATE_IMPL = "_CN20cjthreadstate.noheap21CJRT_CJThreadStateRunHv"
ENGINE.ROOTS = (STATE_IMPL,)
ENGINE.PRODUCTION = {}
ENGINE.NATIVE = ("CJThreadStateObserve",)
ENGINE.PROJECT_PREFIXES = (
    "_CN20cjthreadstate.noheap", "_CN8rt.sched",
    "_CGP20cjthreadstate.noheap", "_CGP8rt.sched",
)
...
def check_state_contract(pre_definitions, final_definitions, object_definitions):
    for stage, definitions in (("pre", pre_definitions), ("final", final_definitions)):
        wrapper = definitions.get("CJRT_CJThreadStateRun")
        body = definitions.get(STATE_IMPL)
        if wrapper is None or body is None:
            raise ENGINE.ClosureError(f"{stage} state root absent")
        if ENGINE.ir_calls("CJRT_CJThreadStateRun", wrapper) != [STATE_IMPL]:
            raise ENGINE.ClosureError(f"{stage} exported wrapper target mismatch")
        calls = [line for line in body.splitlines() if "@CJThreadStateObserve" in line]
        if len(calls) != 1:
            raise ENGINE.ClosureError(f"{stage} observer call count={len(calls)}")
        positions = [calls[0].find(token) for token in ("i32 0", "i32 1", "i32 2", "i32 3")]
        if any(position < 0 for position in positions) or positions != sorted(positions):
            raise ENGINE.ClosureError(f"{stage} state constants absent or out of order")
        zeroing = "@memset" if stage == "pre" else "@llvm.memset.p0i8.i64"
        if body.count(zeroing) != 1 or (stage == "final" and "i8 0" not in body):
            raise ENGINE.ClosureError(f"{stage} owner zeroing count/value mismatch")
    observer_bodies = [body for key, body in object_definitions.items()
        if key[1] == "CJThreadStateObserve"]
    if len(observer_bodies) != 1:
        raise ENGINE.ClosureError(f"object observer definition count={len(observer_bodies)}")


ENGINE.check_atomic_contract = check_state_contract
...
def main():
    ...
    with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
        result = ENGINE.main()
    output = stdout.getvalue().replace("ATOMICSPINLOCK", "CJTHREAD_STATE")
    ...
    sys.stderr.write(stderr.getvalue().replace("ATOMICSPINLOCK", "CJTHREAD_STATE"))
    return result
