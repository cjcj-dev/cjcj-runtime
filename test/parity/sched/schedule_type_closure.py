...
SPEC = importlib.util.spec_from_file_location("schedule_type_closure_engine", ENGINE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"cannot load closure engine: {ENGINE_PATH}")
ENGINE = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(ENGINE)

ROOT_IMPL = "_CN19scheduletype.noheap20CJRT_ScheduleTypeRunHv"
ENGINE.ROOTS = (ROOT_IMPL,)
ENGINE.PRODUCTION = {}
ENGINE.NATIVE = ("ScheduleTypeObserve",)
ENGINE.PROJECT_PREFIXES = (
    "_CN19scheduletype.noheap", "_CN8rt.sched",
    "_CGP19scheduletype.noheap", "_CGP8rt.sched",
)
...
def check_schedule_type_contract(pre_definitions, final_definitions, object_definitions):
    for stage, definitions in (("pre", pre_definitions), ("final", final_definitions)):
        wrapper = definitions.get("CJRT_ScheduleTypeRun")
        body = definitions.get(ROOT_IMPL)
        if wrapper is None or body is None:
            raise ENGINE.ClosureError(f"{stage} schedule type root absent")
        if ENGINE.ir_calls("CJRT_ScheduleTypeRun", wrapper) != [ROOT_IMPL]:
            raise ENGINE.ClosureError(f"{stage} exported wrapper target mismatch")
        calls = [line for line in body.splitlines() if "@ScheduleTypeObserve" in line]
        if len(calls) != 1:
...
def main():
    ...
    with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
        result = ENGINE.main()
    output = stdout.getvalue().replace("ATOMICSPINLOCK", "SCHEDULE_TYPE")
    ...
    sys.stderr.write(stderr.getvalue().replace("ATOMICSPINLOCK", "SCHEDULE_TYPE"))
    return result
