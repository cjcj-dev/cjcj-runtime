#!/usr/bin/env bash
# Fail-closed schedule.h:415-420 value, owner-layout, and full-closure parity.
set -euo pipefail
...
fail()
{
    echo "run_cjthread_state_probe: FAIL $*" >&2
    exit 1
}
...
require_inputs()
{
    ...
    for input in "$SCHEDULE_HEADER" "$CJTHREAD_HEADER" "$SCHEDULE_SOURCE" \
        ...
        "$ROOT/test/parity/sched/cjthread_state_probe.cj" \
        "$ROOT/test/parity/sched/cjthread_state_noheap_roots.cj" \
        "$ROOT/test/parity/sched/cjthread_state_source_check.py" \
        "$ROOT/test/parity/sched/cjthread_state_consumers.txt" \
        "$ROOT/test/parity/sched/cjthread_state_noheap_manifest.txt" \
        "$ROOT/test/parity/sched/cjthread_state_closure.py" \
...
require_inputs
check_compiler
echo "CJTHREAD_STATE_COMPILER path=$SELFHOST_CJC source=$COMPILER_SOURCE sha256=$COMPILER_SHA size=$COMPILER_SIZE status=PASS"
disk_before=$(df -Pk / | awk 'NR==2 {print $4}')
echo "CJTHREAD_STATE_DISK_BEFORE available_kb=$disk_before"

IMP=$(mktemp -d "${TMPDIR:-/tmp}/rt_cjthread_state.XXXXXX")
trap 'rm -rf "$IMP"' EXIT
ulimit -c 0
mkdir -p "$IMP/sched_temps" "$IMP/root_temps" "$IMP/cjthreadstate.noheap"
cp "$ROOT/test/parity/sched/cjthread_state_noheap_roots.cj" \
    "$IMP/cjthreadstate.noheap/roots.cj"

CPP_ORACLE="$IMP/cjthread_state_oracle"
CJ_PROBE="$IMP/cjthread_state_probe"
...
build_cpp_oracle()
{
    g++ -std=c++14 -O2 "${CPP_WARN[@]}" "${CPP_SELECT[@]}" "${CPP_INCLUDE[@]}" \
        -DCJTHREAD_STATE_CPP_ORACLE "$ROOT/test/parity/sched/cjthread_state_ref.cpp" \
        -o "$CPP_ORACLE"
...
check_transcript()
{
    local transcript=$1 zero_bytes
    [[ $(wc -l < "$transcript") -eq 7 ]] || fail "state transcript line count mismatch"
    grep -Fxq 'LUA_CJTHREAD_STATE_REP enum_size=4 enum_align=4 enum_underlying_signed=0 storage_size=4 storage_align=4 storage_signed=1 signed_minus_one=-1 signed_minus_one_bytes=ffffffff' "$transcript" || fail "signed representation mismatch"
    grep -Fxq 'LUA_CJTHREAD_STATE name=LUA_CJTHREAD_INIT value=0 bytes=00000000' "$transcript" || fail "INIT mismatch"
    grep -Fxq 'LUA_CJTHREAD_STATE name=LUA_CJTHREAD_SUSPENDING value=1 bytes=01000000' "$transcript" || fail "SUSPENDING mismatch"
    grep -Fxq 'LUA_CJTHREAD_STATE name=LUA_CJTHREAD_RUNNING value=2 bytes=02000000' "$transcript" || fail "RUNNING mismatch"
    grep -Fxq 'LUA_CJTHREAD_STATE name=LUA_CJTHREAD_DONE value=3 bytes=03000000' "$transcript" || fail "DONE mismatch"
    grep -Fxq 'LUA_CJTHREAD_LAYOUT sizeof=200 align=8 sem=32 state=64 attrUser=68' "$transcript" || fail "LuaCJThread owner layout mismatch"
    zero_bytes=$(printf '00%.0s' {1..200})
    grep -Fxq "LUA_CJTHREAD_OWNER_BYTES bytes=$zero_bytes" "$transcript" || fail "complete owner initialization bytes mismatch"
}
...
check_source_and_consumers()
{
    ...
    [[ $(rg -n 'LUA_CJTHREAD_(INIT|SUSPENDING|RUNNING|DONE)' "$SCHEDULE_SOURCE" | wc -l) -eq 12 ]] ||
        fail "named C++ consumer reference count mismatch"
    local class line expected actual rows=0
    declare -A counts=([store]=0 [compare]=0 [switch_label]=0 [return]=0)
    while IFS='|' read -r class line expected; do
        [[ -n "$class" && ${class:0:1} != '#' ]] || continue
        actual=$(sed -n "${line}p" "$SCHEDULE_SOURCE")
        [[ "$actual" == "$expected" ]] || fail "consumer drift at cjthread.cpp:$line"
        ((++rows)); ((++counts[$class]))
    done < "$ROOT/test/parity/sched/cjthread_state_consumers.txt"
    [[ $rows -eq 13 && ${counts[store]} -eq 6 && ${counts[compare]} -eq 4 &&
       ${counts[switch_label]} -eq 2 && ${counts[return]} -eq 1 ]] ||
        fail "consumer classification count mismatch"
    echo "CJTHREAD_STATE_CONSUMERS total=13 store=6 compare=4 switch_label=2 return=1 ported_functions=0 status=PASS"
    echo "CJTHREAD_STATE_PLATFORMS target_cpp_branches=0 linux_x86_64=COMPILED_EXECUTED macos=UNEXECUTED_DEBT ios=UNEXECUTED_DEBT aarch64_linux=UNEXECUTED_DEBT android_arm32=UNEXECUTED_DEBT android_arm64=UNEXECUTED_DEBT win64=UNEXECUTED_DEBT ohos=UNEXECUTED_DEBT other=UNEXECUTED_DEBT status=PASS"
}
...
build_cangjie_probe()
{
    ...
    run_cjc --package "$IMP/cjthreadstate.noheap" --output-type=staticlib -O2 \
        ...
        --output-dir "$IMP" -o libcjthreadstate.noheap.a
    g++ -std=c++14 -O2 -fno-inline -fno-toplevel-reorder -fPIC "${CPP_WARN[@]}" \
        "${CPP_SELECT[@]}" "${CPP_INCLUDE[@]}" \
        -c "$ROOT/test/parity/sched/cjthread_state_ref.cpp" -o "$IMP/cjthread_state_ref.o"
    ...
    run_cjc "$ROOT/test/parity/sched/cjthread_state_probe.cj" --import-path "$IMP" \
        --int-overflow wrapping "$IMP/libcjthreadstate.noheap.a" "$IMP/librt.sched.a" \
        "$IMP/cjthread_state_ref.o" "$IMP/CJThreadSemaphore.o" "$IMP/CJThreadSpinLock.o" \
...
check_ir_and_closure()
{
    "$LLVM_BIN/llvm-link" "$IMP/sched_temps/rt.sched.bc" \
        "$IMP/root_temps/cjthreadstate.noheap.bc" -o "$IMP/linked.pre.bc"
    "$LLVM_BIN/llvm-link" "$IMP/sched_temps/rt.sched.opt.bc" \
        "$IMP/root_temps/cjthreadstate.noheap.opt.bc" -o "$IMP/linked.final.bc"
    "$LLVM_BIN/llvm-dis" "$IMP/linked.pre.bc" -o "$IMP/linked.pre.ll"
    "$LLVM_BIN/llvm-dis" "$IMP/linked.final.bc" -o "$IMP/linked.final.ll"
    grep -Fxq '%"record.rt.sched:LuaCJThread" = type { i8*, i8*, i8*, i8*, %"record.rt.sched:Semaphore", i32, %"record.rt.sched:CJThreadAttr" }' "$IMP/linked.final.ll" ||
        fail "LuaCJThread final IR record mismatch"
    closure_args=(
        --pre-ll "$IMP/linked.pre.ll"
        --final-ll "$IMP/linked.final.ll"
        --manifest "$ROOT/test/parity/sched/cjthread_state_noheap_manifest.txt"
        --object "$IMP/root_temps/cjthreadstate.noheap.o"
        --object "$IMP/sched_temps/rt.sched.o"
        --object "$IMP/cjthread_state_ref.o"
        --object "$IMP/CJThreadSemaphore.o"
        --object "$IMP/CJThreadSpinLock.o"
    )
    local closure_output
    closure_output=$(python3 "$ROOT/test/parity/sched/cjthread_state_closure.py" "${closure_args[@]}")
    printf '%s\n' "$closure_output"
    grep -Eq 'CJTHREAD_STATE_PRE_CLOSURE reached_defs=([0-9]+) scanned_defs=\1 ' <<< "$closure_output" || fail "pre closure equality absent"
    grep -Eq 'CJTHREAD_STATE_FINAL_CLOSURE reached_defs=([0-9]+) scanned_defs=\1 ' <<< "$closure_output" || fail "final closure equality absent"
    grep -Eq 'CJTHREAD_STATE_OBJECT_CLOSURE reached_defs=([0-9]+) scanned_defs=\1 ' <<< "$closure_output" || fail "object closure equality absent"
}
...
run_negative_injections()
{
    ...
    for mode in value_init value_suspending value_running value_done swapped omitted alias64 initializer; do
        ...
        grep -Fq "CJTHREAD_STATE_SOURCE FAIL mode=$mode" "$IMP/source-negative.$mode.log" ||
            fail "source negative did not execute checker mode=$mode"
        echo "CJTHREAD_STATE_NEGATIVE mode=$mode rc=$rc stage=source reason=representation_or_inventory status=PASS"
    done
    for mode in missing extra allocation_pre allocation_final allocation_object \
        ...
        set +e
        python3 "$ROOT/test/parity/sched/cjthread_state_closure.py" "${closure_args[@]}" \
            --mode "$mode" > "$IMP/closure-negative.$mode.log" 2>&1
        rc=$?
        set -e
        [[ $rc -ne 0 ]] || fail "closure negative accepted mode=$mode"
        grep -Fq "CJTHREAD_STATE_CLOSURE FAIL mode=$mode" "$IMP/closure-negative.$mode.log" ||
        ...
        echo "CJTHREAD_STATE_NEGATIVE mode=$mode rc=$rc stage=$stage symbol=$symbol reason=$reason status=PASS"
    done
}
...
echo "CJTHREAD_STATE_TRANSCRIPT lines=$(wc -l < "$CJ_TRANSCRIPT") bytes=$(wc -c < "$CJ_TRANSCRIPT") sha256=$(sha256sum "$CJ_TRANSCRIPT" | awk '{print $1}') cmp=PASS"
cat "$CJ_TRANSCRIPT"
check_ir_and_closure
run_negative_injections

echo "CJTHREAD_STATE_BINARIES oracle_sha256=$(sha256sum "$CPP_ORACLE" | awk '{print $1}') oracle_size=$(stat -c %s "$CPP_ORACLE") cj_sha256=$(sha256sum "$CJ_PROBE" | awk '{print $1}') cj_size=$(stat -c %s "$CJ_PROBE") status=PASS"
echo "CJTHREAD_STATE_STAGES cpp_header=1 cj_values=4 signed_storage=1 owner_layout=1 owner_bytes=200 source_inventory=1 consumers=13 pre_closure=1 final_closure=1 object_closure=1 allocation_negatives=3 barrier_negatives=3 definition_negatives=2 source_negatives=8 status=PASS"
disk_after=$(df -Pk / | awk 'NR==2 {print $4}')
disk_delta=$((disk_before - disk_after))
echo "CJTHREAD_STATE_DISK_AFTER available_kb=$disk_after consumed_kb=$disk_delta"
(( disk_delta < 2097152 )) || fail "probe consumed more than 2GiB"
echo "run_cjthread_state_probe: PASS"
