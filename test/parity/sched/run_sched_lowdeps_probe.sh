#!/usr/bin/env bash
# Fail-closed CJThread list, stack foundation, and timer foundation parity.
set -euo pipefail
...
trap 'rm -rf "$TMP"' EXIT
mkdir -p "$TMP/sched_temps" "$TMP/root_temps" "$TMP/schedlowdeps.noheap" "$TMP/win.release" "$TMP/win.debug"
cp "$ROOT/test/parity/sched/sched_lowdeps_noheap_roots.cj" "$TMP/schedlowdeps.noheap/roots.cj"
...
g++ -std=c++14 -O2 "${CPP_WARN[@]}" "${CPP_SELECT[@]}" "${CPP_INCLUDE[@]}" \
    -DSCHED_LOWDEPS_CPP_ORACLE "$ROOT/test/parity/sched/sched_lowdeps_ref.cpp" -o "$TMP/oracle"
...
run_cjc --package "$TMP/schedlowdeps.noheap" --output-type=staticlib -O2 \
    --int-overflow wrapping --import-path "$TMP" --save-temps "$TMP/root_temps" \
    --output-dir "$TMP" -o libschedlowdeps.noheap.a
g++ -std=c++14 -O2 -fno-inline -fno-toplevel-reorder -fPIC "${CPP_WARN[@]}" \
    "${CPP_SELECT[@]}" "${CPP_INCLUDE[@]}" \
    -c "$ROOT/test/parity/sched/sched_lowdeps_ref.cpp" -o "$TMP/ref.o"
...
run_cjc "$ROOT/test/parity/sched/sched_lowdeps_probe.cj" --import-path "$TMP" \
    --int-overflow wrapping "$TMP/libschedlowdeps.noheap.a" "$TMP/librt.sched.a" \
...
[[ $(rg -c '^public func Link(Init|Remove|PushHead|PushTail|IsEmpty)' "$ROOT/src/rt.sched/List.cj") -eq 5 ]] || fail "Link inventory"
[[ $(rg -c '^public struct (CJThreadStack|StackInfo|ArgAttr|StackAttr)' "$ROOT/src/rt.sched/CJThreadStack.cj") -eq 4 ]] || fail "stack record inventory"
[[ $(rg -c '^public const ERROR_TIMER_' "$ROOT/src/rt.sched/Timer.cj") -eq 12 ]] || fail "timer error inventory"
[[ $(rg -c '^public const TIMER_(IDLE|WAITING|RUNNING|STOPPED|MODIFYING|MODIFIED_AHEAD|MODIFIED_DELAYED|REMOVING|REMOVED|MOVING)' "$ROOT/src/rt.sched/Timer.cj") -eq 10 ]] || fail "timer state inventory"
! rg -q '^public func Timer(New|Stop|Reset|Sleep|Release|Exit|SchmonCheck|CheckReady|CheckExistence|TryStop|Num)' "$ROOT/src/rt.sched/Timer.cj" || fail "timer callable stub invented"
grep -Fq 'TIMER-SCHEDULE-OWNER' "$ROOT/src/rt.sched/Timer.cj" || fail "timer owner debt absent"
grep -Fq 'CJTHREAD-STACK-ALLOC-LIFETIME' "$ROOT/src/rt.sched/CJThreadStack.cj" || fail "stack owner debt absent"
grep -Fq 'std::atomic<unsigned int> head __attribute__((aligned(64)))' "$CJTHREAD_ROOT/runtime/util/queue/include/queue.h" || fail "queue alignment blocker drift"
echo "SCHED_LOWDEPS_SOURCE dulink_functions=12 link_functions=5 stack_records=4 timer_errors=12 timer_states=10 timer_callable_stubs=0 status=PASS"
...
echo "SCHED_LOWDEPS_PLATFORM target=Linux-OHOS compile=PASS execute=PASS status=PASS"
echo "SCHED_LOWDEPS_PLATFORM target=Apple source_branch=COMMON native_execute=DEBT-APPLE-SDK status=EXPLICIT-DEBT"
echo "SCHED_LOWDEPS_PLATFORM target=Win64 cj_release=PASS cj_debug=PASS status=PASS"

"$LLVM_BIN/llvm-link" "$TMP/sched_temps/rt.sched.bc" "$TMP/root_temps/schedlowdeps.noheap.bc" -o "$TMP/pre.bc"
"$LLVM_BIN/llvm-link" "$TMP/sched_temps/rt.sched.opt.bc" "$TMP/root_temps/schedlowdeps.noheap.opt.bc" -o "$TMP/final.bc"
...
closure_args=(--pre-ll "$TMP/pre.ll" --final-ll "$TMP/final.ll"
    --manifest "$ROOT/test/parity/sched/sched_lowdeps_noheap_manifest.txt"
    --object "$TMP/root_temps/schedlowdeps.noheap.o"
    --object "$TMP/sched_temps/rt.sched.o" --object "$TMP/ref.o"
    --object "$TMP/semaphore.o" --object "$TMP/spinlock.o")
closure_output=$(python3 "$ROOT/test/parity/sched/sched_lowdeps_closure.py" "${closure_args[@]}")
printf '%s\n' "$closure_output"
grep -Eq 'SCHED_LOWDEPS_PRE_CLOSURE reached_defs=([0-9]+) scanned_defs=\1 ' <<< "$closure_output" || fail "pre closure"
grep -Eq 'SCHED_LOWDEPS_FINAL_CLOSURE reached_defs=([0-9]+) scanned_defs=\1 ' <<< "$closure_output" || fail "final closure"
grep -Eq 'SCHED_LOWDEPS_OBJECT_CLOSURE reached_defs=([0-9]+) scanned_defs=\1 ' <<< "$closure_output" || fail "object closure"

for mode in missing extra allocation_pre allocation_final allocation_object barrier_pre barrier_final barrier_object; do
    set +e
    python3 "$ROOT/test/parity/sched/sched_lowdeps_closure.py" "${closure_args[@]}" \
        --mode "$mode" > "$TMP/negative.$mode" 2>&1
    rc=$?
    set -e
    [[ $rc -ne 0 ]] || fail "negative accepted mode=$mode"
    grep -Fq "SCHED_LOWDEPS_CLOSURE FAIL mode=$mode" "$TMP/negative.$mode" || fail "negative path mode=$mode"
    echo "SCHED_LOWDEPS_NEGATIVE mode=$mode rc=$rc status=PASS"
done

echo "SCHED_LOWDEPS_BINARIES oracle_sha256=$(sha256sum "$TMP/oracle" | awk '{print $1}') cj_sha256=$(sha256sum "$TMP/probe" | awk '{print $1}') status=PASS"
echo "run_sched_lowdeps_probe: PASS"
